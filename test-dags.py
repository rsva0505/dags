...
GCP_PROJECT_ID = "apachegke"  # ¡REEMPLAZA ESTO!
...
with DAG(
    dag_id="gcs_read_write_verification",
    start_date=pendulum.datetime(2023, 1, 1, tz="UTC"),
    catchup=False,
    schedule=None,
    tags=["gcs", "verification", "test"],
    description="DAG para verificar lectura y escritura en GCS",
) as dag:
    # 
    ...
    check_file_existence_in_gcs = GCSObjectExistenceSensor(
        ...
        project_id=GCP_PROJECT_ID,
        mode="poke",  # 'poke' para polling continuo hasta que se cumpla
        poke_interval=5,  # Cada 5 segundos
        timeout=60,  # Fallar después de 60 segundos si no se encuentra
    )
    ...
    delete_file_from_gcs = BashOperator(
        ...
        # O usar GCSDeleteObjectOperator si prefieres un operador de Python
